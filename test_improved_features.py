#!/usr/bin/env python3
"""
Test improved audio features with low thresholds and logarithmic transforms
"""
...
def test_improved_features():
    ...
    # 1. Low threshold ratio (25th percentile)
    low_threshold = np.percentile(sample_audio, 25)
    low_ratio = np.mean(sample_audio < low_threshold)
    print(f"   Low threshold (25th percentile): {low_threshold:.2f}")
    print(f"   Low threshold ratio: {low_ratio:.4f}")
    
    # 2. High spike ratio (90th percentile)
    high_threshold = np.percentile(sample_audio, 90)
    high_ratio = np.mean(sample_audio > high_threshold)
    print(f"   High threshold (90th percentile): {high_threshold:.2f}")
    print(f"   High spike ratio: {high_ratio:.4f}")
    ...
    print(f"\n✅ Improved feature extraction test completed!")
    print(f"   Total features: {len(feature_names)}")
    print(f"   Audio features: {len(audio_features)}")
    print(f"   New features include:")
    print(f"     - Low threshold ratios (25th percentile)")
    print(f"     - High spike ratios (90th percentile)")
    print(f"     - Logarithmic transforms (log_mean, log_std, log_max)")
